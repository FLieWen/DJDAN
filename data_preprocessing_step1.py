...
#         # 假设数据存储在 'data' 和 'label' 的字段中
#         data = mat_data['data']  # 形状为: (num_samples, num_electrodes, num_timepoints)
#         labels = mat_data['label'].flatten()  # 标签
#         num_samples, num_electrodes, num_timepoints = data.shape
        
#         # 提取3到7秒的数据，假设采样率为1000 Hz
#         start_time = 3 * fs  # 3秒对应的时间点索引
#         end_time = 7 * fs  # 7秒对应的时间点索引
#         data = data[:, :, start_time:end_time]  # 截取时间段，数据形状变为 (num_samples, num_electrodes, 4000)
