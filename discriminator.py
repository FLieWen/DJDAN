...
# # 定义全局鉴别器，只接收特征 fs 和 ft
# class GlobalDiscriminator(nn.Module):
#     def __init__(self):
#         super(GlobalDiscriminator, self).__init__()
#         self.conv = nn.Conv1d(40, 2, kernel_size=61, stride=1)
#         self.softmax = nn.Softmax(dim=1)
...
# # 定义局部鉴别器，接收组合后的特征和分类结果 fs ⊗ ps 和 ft ⊗ pt
# class LocalDiscriminator(nn.Module):
#     def __init__(self, num_classes):
#         super(LocalDiscriminator, self).__init__()
#         self.conv = nn.Conv1d(40, 2, kernel_size=61, stride=1)
...
#     def forward(self, features, preds, alpha=1.0):
#         x = GradientReversalLayer.apply(features, alpha)
#         # 对特征和分类结果逐元素相乘
#         x_combined = features * preds.unsqueeze(2)
#         local_out = []
#         for i in range(self.num_classes):
#             mask = (preds.argmax(dim=1) == i).float().unsqueeze(1)
#             local_out.append(self.conv(x_combined) * mask)
#         return self.softmax(torch.sum(torch.stack(local_out), dim=0))
...
# # # 数据加载
# # source_data_path = 'data/BCIIV2b_mat/Processed_T_BCIIV2b_mat'
# # target_data_path = 'data/BCIIV2b_mat/Processed_E_BCIIV2b_mat'

# # source_loader = DataLoader(datasets.ImageFolder(source_data_path, transform=transforms.ToTensor()), batch_size=64, shuffle=True)
# # target_loader = DataLoader(datasets.ImageFolder(target_data_path, transform=transforms.ToTensor()), batch_size=64, shuffle=True)

# # 超参数
# num_classes = 2  # 假设为二分类任务

# # 初始化鉴别器
# global_discriminator = GlobalDiscriminator()
# local_discriminator = LocalDiscriminator(num_classes)

# # 优化器
# optimizer_gd = optim.Adam(global_discriminator.parameters(), lr=0.0001)
# optimizer_ld = optim.Adam(local_discriminator.parameters(), lr=0.0001)
...
#         # 源域和目标域的标签 (1表示目标域，0表示源域)
#         source_labels = torch.zeros(source_features.size(0), 1)
#         target_labels = torch.ones(target_features.size(0), 1)
        
#         # 全局鉴别器训练
#         global_preds_source = global_discriminator(source_features)
#         global_preds_target = global_discriminator(target_features)
        
#         loss_gd = criterion(global_preds_source, source_labels) + criterion(global_preds_target, target_labels)
#         optimizer_gd.zero_grad()
#         loss_gd.backward()
#         optimizer_gd.step()

#         # 局部鉴别器训练，结合分类器输出和特征逐元素相乘
#         local_preds_source = local_discriminator(source_features, source_preds)
#         local_preds_target = local_discriminator(target_features, target_preds)

#         loss_ld = criterion(local_preds_source, source_labels) + criterion(local_preds_target, target_labels)
#         optimizer_ld.zero_grad()
#         loss_ld.backward()
#         optimizer_ld.step()

#     print(f'Epoch [{epoch+1}/100], Global Discriminator Loss: {loss_gd.item()}, Local Discriminator Loss: {loss_ld.item()}')